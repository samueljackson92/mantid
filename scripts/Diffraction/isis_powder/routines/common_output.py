...
def split_into_tof_d_spacing_groups(run_details, processed_spectra, inst_prefix):
    d_spacing_output = []
    tof_output = []
    run_number = str(run_details.output_run_string)

    d_spacing_group_name = inst_prefix + run_number + "-Results-D-Grp"
    tof_group_name = inst_prefix + run_number + "-Results-TOF-Grp"

    for name_index, ws in enumerate(processed_spectra):
        d_spacing_out_name = d_spacing_group_name + str(name_index + 1)
        tof_out_name = tof_group_name + str(name_index + 1)
...
def save_focused_data(d_spacing_group, tof_group, output_paths, run_number_string, inst_prefix):
    ...
    mantid.SaveNexusProcessed(InputWorkspace=d_spacing_group,
                              Filename=output_paths["nxs_filename_dSpacing"], Append=False)
    mantid.SaveNexusProcessed(InputWorkspace=tof_group, Filename=output_paths["nxs_filename_tof"], Append=False)
